...
class DTLearner(object):
    ...
    def build_tree(self, X, Y):
        ...
        # determine best feature to split on based on correlation between X and Y
        max_corr = 0
        feature_indx = 0
        for i in range(len(X[0])):
            if np.std(X[:, i]) > 0:
                if abs(np.corrcoef(X[:, i], Y)[0, 1]) > max_corr:
                    max_corr = abs(np.corrcoef(X[:, i], Y)[0, 1])
                    feature_indx = i

        #find split value
        splitVal = np.median(X[:,feature_indx])
    ...
    def make_prediction(self, node, x, node_index):
        ...
        if x[int(node[0])] > node[1]: #if value of x at split feature > splitVal , right tree
            ...
            return self.make_prediction(self.DTLearner[node_index], x,node_index)
        else:
    ...
    def add_evidence(self, X, Y):
        ...
        self.DTLearner = self.build_tree(X, Y)
    ...
    def query(self, X_test):
        ...
        prediction =[]
        for x in X_test:
            prediction.append(self.make_prediction(self.DTLearner[0], x, 0))
        return prediction
